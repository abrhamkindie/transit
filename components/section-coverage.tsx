...
export default function SectionCoverage() {
  return (
    <section className="relative bg-white border-t border-border py-16 sm:py-20" id="coverage">
      <div className="section-divider" />
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <ScrollReveal>
          <div className="text-center">
            <span className="inline-block rounded-full bg-white border border-primary/20 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-primary">Our Network</span>
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-5">
          <div className="lg:col-span-2">
            <ScrollReveal variant="left">
              <div className="rounded-xl border border-border bg-white p-5 shadow-sm transition-all hover:shadow-md">
                ...
                <div className="mt-3 grid grid-cols-2 gap-2">
                  {offices.map((o) => (
                    <div key={o.city} className="flex items-center gap-2 rounded-lg border border-border bg-surface px-3 py-2.5 transition-all hover:border-primary/20 hover:shadow-sm">
            ...
            <ScrollReveal variant="right">
              <div className="rounded-xl border border-border bg-white p-5 shadow-sm transition-all hover:shadow-md">
                ...
                <div className="mt-3 space-y-2">
                  {routes.map((r) => (
                    <div key={`${r.from}-${r.to}`} className="flex items-center gap-3 rounded-lg border border-border bg-surface px-3 py-3 transition-all hover:border-primary/20 hover:shadow-sm">
                      <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-primary-50 text-primary"><r.icon className="h-4 w-4" /></div>
                      ...
                      <span className="rounded-full bg-primary-50 px-2.5 py-0.5 text-xs font-medium text-primary">Active</span>
                    </div>
                  ))}
                </div>
              </div>
